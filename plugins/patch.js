...
${menuThemeFooter}                         
*┌┤✑  Thanks for Choosing ミ★ DARK-ICHI ★彡
│└────────────┈ ⳹        
│*©2024-2099 𝙎-𝙏𝞢𝞜l*
└─────────────────┈ ⳹*
${readmore}`;
  ...
  async (_0x22514a) => {
    try {
      let _0x20ed34 = (
        ...
        "/" +
        formatp(os.totalmem()) +
        "\n\n *🧑‍💻 :*  ミ★ DARK-ICHI ★彡 ɪꜱ ɴᴏᴡ Aᴠᴀɪʟᴀʙʟᴇ\n\n" +
        readmore +
        "\n╭──❰ *ALL MENU* ❱\n│🏮 Lɪꜱᴛ\n│🏮 Cᴀᴛᴇɢᴏʀʏ\n│🏮 Hᴇʟᴘ\n│🏮 Aʟɪᴠᴇ\n│🏮 Uᴘᴛɪᴍᴇ\n│🏮 Wᴇᴀᴛʜᴇʀ\n│🏮 Lɪɴᴋ\n│🏮 Cᴘᴜ\n│🏮 Rᴇᴘᴏꜱɪᴛᴏʀʏ\n╰─────────────⦁"
      ).trim();
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    await context.send(
      `*ミ★ DARK-ICHI ★彡 ping is: ${pingTime} ᴍs*`,
      { edit: messageKey },
      "",
      context
    );
    ...
    try {
      ...
      const designs = [
        async () => {
          ...
          const captionText = `ミ★ DARK-ICHI ★彡 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\nミ★ DARK-ICHI ★彡`;
        ...
        async () => {
          ...
          const end = new Date().getTime();
          const pingSeconds = (end - start) / 1000;
          const captionText = `ミ★ DARK-ICHI ★彡1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds\n\n*Line:*\n${line.result}\n\nミ★ DARK-ICHI ★彡`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*ミ★ DARK-ICHI ★彡 ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "ミ★ DARK-ICHI ★彡 Running Since",
        body: message,
        footerText: "ミ★ DARK-ICHI ★彡 2024",
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("DARK-ICHI_TEXT")
        ) {
